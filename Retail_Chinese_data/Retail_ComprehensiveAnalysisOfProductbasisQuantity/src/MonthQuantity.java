...
public class MonthQuantity
{
			...
			Mapper<LongWritable, Text, Text, Text> 
	{
		public void map(LongWritable key, Text value, Context context)
			...
			context.write(new Text(parts[5]), new Text("nov\t" +parts[6]));
		}
	}
			...
			Mapper<LongWritable, Text, Text, Text>
	{
		public void map(LongWritable key, Text value, Context context)
			...
			String record = value.toString();
			String[] parts = record.split(";");
			context.write(new Text(parts[5]), new Text("dec\t" +parts[6]));
		}
	}
...
public void map(LongWritable key, Text value, Context context)
	...
	String record = value.toString();
	String[] parts = record.split(";");
	context.write(new Text(parts[5]), new Text("jan\t" + parts[6]));
		}
	}
...
public void map(LongWritable key, Text value, Context context)
	...
	String record = value.toString();
	String[] parts = record.split(";");
	context.write(new Text(parts[5]), new Text("feb\t" + parts[6]));
		}
	}
	...
	public static class ReduceJoinReducer extends Reducer<Text, Text, Text, Text> {
		public void reduce(Text key, Iterable<Text> values, Context context)
			...
			for (Text t : values) {
				...
				{
					novtotal += Integer.parseInt(parts[1]);
					
				} 
				else if (parts[0].equals("dec"))
				{
					dectotal +=Integer.parseInt(parts[1]);
					
				} 
				else if (parts[0].equals("jan"))
				{
					jantotal += Integer.parseInt(parts[1]);
					
				} 
				else if (parts[0].equals("feb"))
				{
					febtotal +=Integer.parseInt(parts[1]);
	...
	public static void main(String[] args) throws Exception {
	    ...
	    job.setJarByClass(MonthQuantity.class);
	    job.setJobName("comprehensiv analysis of a products totalQuantity on month basis");
